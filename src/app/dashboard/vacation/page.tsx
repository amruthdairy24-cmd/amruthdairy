    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-3 space-y-6">
          <form onSubmit={handleVacationSubmit} className="bg-white border border-border/50 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-[16px] font-bold text-slate-800 tracking-tight">Schedule Pause</h2>
            ...
            {totalDays > 0 && (
              ...
              >
                <div className="flex justify-between items-center pb-3 border-b border-slate-200/50">
                  <span className="text-slate-500">Total Paused Days:</span>
                  <span className="font-extrabold text-slate-800 font-mono text-sm">{totalDays} days</span>
                </div>
                <div className="flex justify-between items-center pb-3 border-b border-slate-200/50">
                  <span className="text-slate-500">Statement Credits Earned:</span>
                  <span className="font-extrabold text-emerald-600 font-mono text-sm">+₹{creditPreview.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500">Delivery Resumes:</span>
                  <span className="font-extrabold text-[#014DA4] text-sm flex items-center gap-1">
          ...
          <div className="bg-white border border-border/50 rounded-3xl p-6 shadow-sm space-y-5 text-[12.5px] font-semibold text-slate-500">
            <h3 className="text-[11px] font-extrabold text-slate-450 select-none uppercase tracking-[2.5px] pl-0.5">Vacation Rules</h3>
            ...
            <div className="space-y-4 text-left leading-relaxed">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm">One Day Advance Notice</p>
                  <p className="text-slate-450 mt-1">Vacation pauses must be submitted at least one day in advance. Same-day pauses are not supported.</p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm">Earn Statement Credits</p>
                  <p className="text-slate-450 mt-1">You will receive a statement credit of <strong className="text-emerald-600 font-extrabold">₹{dailyRate.toFixed(2)}</strong> per day during the pause, reducing your next month's bill.</p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm">Seamless Resume</p>
                  <p className="text-slate-450 mt-1">Delivery will auto-resume the morning after your vacation ends. No reactivation or manual restart needed!</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-border/50 rounded-3xl shadow-sm overflow-hidden flex flex-col justify-between">
            <div className="p-5 border-b border-border/50 bg-slate-50/60 text-left select-none">
              <h3 className="text-[13px] font-black text-[#014DA4] uppercase tracking-wider font-display">Active Vacation Pauses</h3>
              <p className="text-[10px] text-slate-400 font-semibold mt-0.5 uppercase tracking-widest">Confirmed Scheduled breaks</p>
            </div>
            
            <div className="p-2">
              {vacationList.length === 0 ? (
                <div className="text-center py-10 px-4">
                  <Palmtree size={32} className="text-slate-350 mx-auto mb-2.5 stroke-[1.5]" />
                ...
                <div className="space-y-2">
                  {vacationList.map((vac, i) => (
                    <div key={i} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex flex-col gap-3">
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black uppercase text-[#014DA4] bg-sky-500/10 px-2 py-0.5 rounded-full tracking-wider">
                          Vacation Slot
                        </span>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-[11px] text-slate-400 font-semibold">Est. Credit</p>
                          <p className="text-sm font-black text-emerald-650 font-mono mt-0.5">₹{((vac.total_days || 1) * dailyRate).toFixed(2)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
