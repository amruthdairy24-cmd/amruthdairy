...
export function AdminExtraMilkModal({ isOpen, onClose, onSuccess, customerId, customerName }: AdminExtraMilkModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/customer-actions/extra-milk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customer_id: customerId, order_date: orderDate, extra_litres: extraLitres })
      })
        ...
        >
          <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">Extra Milk</h2>
          ...
          <div className="p-5">
            ...
            <form id="admin-extra-form" onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-slate-500 mb-1.5">Date</label>
                <input 
                  required
                  type="date" 
                  value={orderDate}
                  onChange={e => setOrderDate(e.target.value)}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 dark:bg-slate-950 dark:border-slate-800 dark:text-white transition-all"
                />
              </div>
              <div>
                ...
                <select 
                  required
                  value={extraLitres}
                  onChange={e => setExtraLitres(Number(e.target.value))}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 dark:bg-slate-950 dark:border-slate-800 dark:text-white transition-all"
                >
              ...
              <p className="text-[11px] text-slate-500 font-medium">Charges will be logged to their billing balance automatically.</p>
            </form>
          </div>
          ...
          <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex gap-3">
            ...
            <button
              type="submit"
              form="admin-extra-form"
              disabled={loading}
              className="flex-1 px-4 py-3 rounded-xl font-bold text-sm bg-cyan-500 hover:bg-cyan-600 text-white transition-colors shadow-lg shadow-cyan-500/20 disabled:opacity-50 flex justify-center items-center"
            >
