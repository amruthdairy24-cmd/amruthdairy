  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/customer-actions/skip', {
        ...
        body: JSON.stringify({ customer_id: customerId, skip_date: skipDate })
      })

      const data = await res.json()
      
      if (!res.ok) {
        throw new Error(data.message || 'Failed to mark skip')
      }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="p-5">
            ...
            <form id="admin-skip-form" onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-slate-500 mb-1.5">Skip Date</label>
                <input 
                  required
                  type="date" 
                  value={skipDate}
                  onChange={e => setSkipDate(e.target.value)}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-slate-950 dark:border-slate-800 dark:text-white transition-all"
                />
              </div>
              <p className="text-[11px] text-slate-500 font-medium">Note: Admins bypass the 9 PM cutoff. Skips will automatically generate credits for next month.</p>
            </form>
          </div>
          ...
          <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex gap-3">
            ...
            <button
              type="submit"
              form="admin-skip-form"
              disabled={loading}
              className="flex-1 px-4 py-3 rounded-xl font-bold text-sm bg-amber-500 hover:bg-amber-600 text-white transition-colors shadow-lg shadow-amber-500/20 disabled:opacity-50 flex justify-center items-center"
            >
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Confirm Skip'}
            </button>
          </div>
        </motion.div>
      </div>
